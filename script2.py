...
stores = pd.merge(stores, tmp, how='left', on=['air_store_id','dow']) 
tmp2=data['tra'].groupby(['air_store_id']).agg({'visitors' : [np.mean,np.std]}).reset_index()
tmp2.columns = ['air_store_id','mean_visitorso','std_devo']
stores = pd.merge(stores, tmp2, how='left', on=['air_store_id'])
...
predict_y = xg.predict(test_x)
test['visitors'] = np.expm1(predict_y)
test[['id', 'visitors']].to_csv(
    'submission.csv', index=False, float_format='%.3f')